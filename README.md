...
Just open index.html in your browser, and it works. (To run things locally, you do not need to launch a web server.)

The state (RAM and FLASH memory) is saved locally using IndexedDB every 8 seconds, so the data is preserved even after a restart.

By right-clicking on the screen or pressing and holding the touch on the screen, you can export and import the state to a file, reset the calculator, and load custom variables from files.

Compared to the original, the ratio of the display to the keyboard has been fixed for maximum comfort.

Many bugs in the emulation of the processor's unsigned operations have been fixed, so FLASH Apps, Jezzball, and others now work as well :-)

If you upload the emulator to your web server, the web page can be added to your iPhone's home screen as a shortcut and it will launch full screen, which allows better use of screen real estate.



## TI-89 Emulator - The version I based this on, some of the information is outdated
